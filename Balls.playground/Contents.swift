...
public class SquareArea: UIView, SquareAreaProtocol {
    ...
    required public init(size: CGSize, color: UIColor) {
        ...
        // строим прямоугольную графическую бласть
        super.init(frame:
                    CGRect(x: 0, y: 0, width: size.width, height: size.height))
        // изменяем цвет фона
        self.backgroundColor = color
        // указываем границы прямоугольной области как объекты взаимодействия чтобы об них могли ударятся шарики
    ...
    public func setBalls(withColors ballsColor: [UIColor], andRadius radius: Int) {
        ...
        for (index, oneBallColor) in ballsColor.enumerated() {
            ...
            // создаем экзмепляр сущности "шарик"
...
//утсановка экземпляра в качестве текущего отображения
PlaygroundPage.current.liveView = area

// создаём экземпляры шары. реализуем метод setBalls


area.setBalls(withColors: [UIColor.blue], andRadius: 55)
area.setBalls(withColors: [UIColor.blue], andRadius: 55)
area.setBalls(withColors: [UIColor.yellow], andRadius: 55)
area.setBalls(withColors: [UIColor.yellow], andRadius: 55)
